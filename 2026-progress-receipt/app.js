(() => {
  ...
  function renderOutro(target, t) {
    ...
    const reveal = easeOut(Math.min(1, t));
  ...
  async function saveVideo() {
    if (!canvas.captureStream || !window.MediaRecorder) {
      exportStatus.textContent = "当前浏览器暂不支持视频生成，请先保存高清图片。";
      return;
    }
    ...
    if (!mimeType) {
      exportStatus.textContent = "当前浏览器没有可用的视频编码器，请保存高清图片。";
      return;
    }

    exportStatus.textContent = "正在生成10秒视频，请不要离开页面…";
    videoButton.disabled = true;
    shareButton.disabled = true;

    try {
      const exportCanvas = document.createElement("canvas");
      exportCanvas.width = BASE_W;
      exportCanvas.height = BASE_H;
      ...
      await new Promise((resolve) => {
        function exportFrame(now) {
          ...
          renderFrame(
            exportContext,
            Math.min(elapsed, VIDEO_DURATION - 1),
            exportCanvas.width,
            exportCanvas.height,
          );
          if (elapsed < VIDEO_DURATION) requestAnimationFrame(exportFrame);
          else resolve();
        }
      ...
      const mp4 = await renderMp4(blob);
  ...
  async function renderMp4(videoBlob) {
    const formData = new FormData();
    formData.append("video", videoBlob, "progress-receipt.webm");
  ...
  function canvasToBlob(target, type) {
    return new Promise((resolve, reject) => {
      target.toBlob((blob) => (blob ? resolve(blob) : reject(new Error("canvas export failed"))), type, 0.96);
    });
  }
